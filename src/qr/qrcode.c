...
static const uint16_t NUM_ERROR_CORRECTION_CODEWORDS_M[10] = {10, 16, 26, 36, 48, 64, 72, 88, 110, 130};
static const uint8_t NUM_ERROR_CORRECTION_BLOCKS_M[10] = {1, 1, 1, 2, 2, 4, 4, 4, 5, 5};
static const uint16_t NUM_RAW_DATA_MODULES[10] = {208, 359, 567, 807, 1079, 1383, 1568, 1936, 2336, 2768};
...
static void drawFinderPattern(BitBucket *modules, BitBucket *isFunction, uint8_t x, uint8_t y) {
    ...
    for (int8_t i = -4; i <= 4; i++) {
        for (int8_t j = -4; j <= 4; j++) {
            ...
            if (0 <= xx && xx < size && 0 <= yy && yy < size) setFunctionModule(modules, isFunction, (uint8_t)xx, (uint8_t)yy, dist != 2 && dist != 4);
        }
    }
}
...
static void drawFormatBits(BitBucket *modules, BitBucket *isFunction, uint8_t ecc, uint8_t mask) {
    ...
    data = (data << 10) | rem;
    data ^= 0x5412U;
    for (uint8_t i = 0; i <= 5; i++) setFunctionModule(modules, isFunction, 8, i, ((data >> i) & 1U) != 0);
    ...
    for (int8_t i = 9; i < 15; i++) setFunctionModule(modules, isFunction, (uint8_t)(14 - i), 8, ((data >> i) & 1U) != 0);
    for (int8_t i = 0; i <= 7; i++) setFunctionModule(modules, isFunction, (uint8_t)(size - 1 - i), 8, ((data >> i) & 1U) != 0);
    for (int8_t i = 8; i < 15; i++) setFunctionModule(modules, isFunction, 8, (uint8_t)(size - 15 + i), ((data >> i) & 1U) != 0);
...
static void drawFunctionPatterns(BitBucket *modules, BitBucket *isFunction, uint8_t version, uint8_t ecc) {
    ...
    if (version > 1) {
        ...
        for (uint8_t i = 0, pos = (uint8_t)(sz - 7); i < alignCount - 1; i++, pos = (uint8_t)(pos - step)) alignPosition[alignPositionIndex--] = pos;
...
static uint32_t getPenaltyScore(BitBucket *modules) {
    ...
    for (uint8_t y = 0; y < size; y++) {
        ...
        for (uint8_t x = 0; x < size; x++) {
            ...
            if (x >= 10) {
                if (bitsRow == 0x05DU || bitsRow == 0x5D0U) result += PENALTY_N3;
                if (bitsCol == 0x05DU || bitsCol == 0x5D0U) result += PENALTY_N3;
            }
            if (color) black++;
        }
    }
...
static int8_t encodeDataCodewords(BitBucket *dataCodewords, const uint8_t *text, uint16_t length, uint8_t version) {
    ...
    for (uint16_t i = 0; i < length; i++) bb_appendBits(dataCodewords, (uint8_t)text[i], 8);
    return MODE_BYTE;
}
...
uint16_t qrcode_getDataCapacityBytes(uint8_t version) {
    if (version < 1 || version > 10) return 0;
    uint16_t moduleCount = NUM_RAW_DATA_MODULES[version - 1];
    return (uint16_t)(moduleCount / 8 - NUM_ERROR_CORRECTION_CODEWORDS_M[version - 1]);
}

int8_t qrcode_initBytes(QRCode *qrcode, uint8_t *modules, uint8_t version, uint8_t ecc, uint8_t *data, uint16_t length) {
    if (ecc != ECC_MEDIUM) return -1;
    ...
    if (length > dataCapacity) return -1;
    ...
    for (uint8_t padByte = 0xEC; codewords.bitOffsetOrWidth < (dataCapacity * 8U); padByte ^= 0xEC ^ 0x11) bb_appendBits(&codewords, padByte, 8);
