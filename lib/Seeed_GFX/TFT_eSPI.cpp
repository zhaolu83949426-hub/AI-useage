...
void TFT_eSPI::initBus(void) {
  ...
  if (TFT_CS >= 0) {
#if defined(OPENDISPLAY_SEEED_GFX_RUNTIME_PINS)
    if (opnd_seeed_runtime_cs >= 0) {
      pinMode(opnd_seeed_runtime_cs, OUTPUT);
      digitalWrite(opnd_seeed_runtime_cs, HIGH);
    }
#else
...
// In parallel mode and with the RP2040 processor, the TFT_WR line is handled in the  PIO
  ...
  if (TFT_RST >= 0) {
    ...
    if (opnd_seeed_runtime_rst >= 0) {
      pinMode(opnd_seeed_runtime_rst, OUTPUT);
...
#if defined(OPENDISPLAY_SEEED_GFX_RUNTIME_PINS)
      spi.begin(opnd_seeed_runtime_sclk, opnd_seeed_runtime_miso, opnd_seeed_runtime_mosi, -1);
#else
